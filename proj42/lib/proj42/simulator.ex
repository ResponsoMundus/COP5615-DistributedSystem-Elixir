defmodule Proj42.Simulator do
    use Agent

    def start_link([num_user, num_msg, debug]) do
        Agent.start_link(fn -> simulate(num_user, num_msg, debug) end)
    end
    ...
    def simulate(num_user, num_msg, debug \\ false) do
        {:ok, _pid} = Supervisor.start_link(
            [
                %{
                    id: Proj42.ActorSupervisor,
                    start: {Proj42.ActorSupervisor, :start_link, [:ok]}
                }
            ],
            [
                strategy: :one_for_all,
                name: Proj42.SimulatorSupervisor
            ]
        )

        server_pid = Proj42.ActorSupervisor.start_server()
        client_pid_list = Proj42.ActorSupervisor.start_clients(num_user, server_pid)
        ...
        new_client_pid_list = Proj42.ActorSupervisor.start_clients(10, server_pid)
        ...
        #Supervisor.stop(Proj42.SimulatorSupervisor)
        #{time_1, time_2, time_3, time_4, time_5, time_6, time_7, time_8, time_9}
        
        {server_pid, client_pid_list}
    end
end
